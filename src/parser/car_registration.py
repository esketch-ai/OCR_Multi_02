...
class CarRegistrationParser:
    ...
    def parse_hybrid(self, hybrid_result, filename=None):
        """
        Parses using both Google and Paddle results and picks the best.
        Uses filename as fallback for vehicle number validation.

        Args:
            hybrid_result: Dictionary with 'google' and 'paddle' OCR results
            filename: Optional filename to extract vehicle number from as fallback
        """
        google_data = hybrid_result.get('google', {})
        paddle_data = hybrid_result.get('paddle', {})

        g_text = google_data.get('text', '')
        g_annotation = google_data.get('annotation', None)

        p_lines = paddle_data.get('text_lines', [])
        p_full_text = "\n".join(p_lines)
        p_raw_result = paddle_data.get('result', [])

        # Parse both
        g_parsed = self.parse(g_text) if g_text else {}
        p_parsed = self.parse(p_full_text) if p_full_text else {}
        ...
        for field in fields:
            g_val = g_parsed.get(field)
            p_val = p_parsed.get(field)

            selected_val = g_val
            selected_conf = 0.0

            # Calculate Confidences
            g_conf = self._calculate_google_confidence(g_val, g_annotation)
            p_conf = self._calculate_paddle_confidence(p_val, p_raw_result)

            # Selection Logic
            if not g_val and p_val:
                selected_val = p_val
                selected_conf = p_conf
                if field == 'vin' or field == 'vehicle_no':
                    logging.info(f"Hybrid: Switched to Paddle for {field} ({p_val}, conf={p_conf:.2f})")
            elif g_val and p_val:
                # If Google low confidence and Paddle high confidence, switch
                if g_conf < 0.8 and p_conf > g_conf and p_val != g_val:
                    selected_val = p_val
                    selected_conf = p_conf
                    logging.info(f"Hybrid: Switched to Paddle for {field} due to higher conf ({p_conf:.2f} vs {g_conf:.2f})")
                else:
                    selected_val = g_val
                    selected_conf = g_conf
            else:
                selected_val = g_val
                selected_conf = g_conf

            final_result[field] = selected_val
            confidences[field] = selected_conf

        # Fallback: Use filename for vehicle_no if OCR failed or produced invalid result
        ocr_vehicle_no = final_result.get('vehicle_no')
        if filename and (not ocr_vehicle_no or not self._is_valid_vehicle_no(ocr_vehicle_no)):
            filename_vehicle_no = self._extract_vehicle_no_from_filename(filename)
            if filename_vehicle_no:
                logging.info(f"Vehicle No from filename: {filename_vehicle_no} (OCR was: '{ocr_vehicle_no}')")
                final_result['vehicle_no'] = filename_vehicle_no
                # Set confidence to 1.0 since filename is reliable
                confidences['vehicle_no'] = 1.0

        # Extract fuel type using multiple sources
        fuel_type = self._determine_fuel_type(
            filename=filename,
            ocr_text=g_text,
            model_name=final_result.get('model_name'),
            engine_type=final_result.get('engine_type')
        )
        final_result['fuel_type'] = fuel_type
        confidences['fuel_type'] = 1.0 if fuel_type != 'Unknown' else 0.0

        final_result['confidences'] = confidences
        return final_result
            ...
            for block in page.blocks:
                for paragraph in block.paragraphs:
                    for word in paragraph.words:
                        ...
                        if len(word_text) > 1 and (word_text in target or target in word_text):
                            w_conf = sum([s.confidence for s in word.symbols]) / len(word.symbols)
                            accum_conf += w_conf
                            count += 1
        
        return (accum_conf / count) if count > 0 else 0.0

    def _calculate_paddle_confidence(self, value, raw_result):
        if not value:
            return 0.0
        target = value.replace(" ", "")
        best_score = 0.0
        if not raw_result: return 0.0
        
        for line in raw_result:
            if not line: continue
            line_text = line[1][0].replace(" ", "")
            line_score = line[1][1]
